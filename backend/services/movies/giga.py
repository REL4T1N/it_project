...
def generate_extended_description(movie: MovieInfo) -> str:
    SYSTEM_PROMPT = (
        ...
        "Тебе даётся расширенная информация о фильме: название, год выпуска, страны, жанры, описание, краткое описание, рейтинги, количество голосов, продолжительность, возрастной рейтинг, постер, актёры, бюджет, кассовые сборы, дата премьеры, связанные фильмы (сиквелы/приквелы), похожие фильмы, возможности просмотра.\n"
        "Используй все доступные данные для создания художественного и подробного описания для каталога фильмов.\n"
